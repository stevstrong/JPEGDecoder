...
#include "jpeg1.h" // Baboon40
#include "jpeg2.h" // Mouse480
//#include "jpeg3.h" // Lena20k
#include "jpeg4.h" // EagleEye
...
void setup()
{
	...
	Serial.println("***** ILI9486 SPI 480x320 JPeg Test *****\n");
	tft.begin();
}
...
/**/
  ...
  tft.fillScreen(random(0xFFFF));
	Serial.println("drawing eagle_eye...");
...
*/
  ...
  tft.fillScreen(random(0xFFFF));
  // This image will be deliberately cropped as it is 480 x 320 this extends off the screen when plotted
...
int32 cnt = 0;
  ...
  while (JpegDec.read()) {    // While there is more data in the file
    ...
    if ((mcu_x + mcu_w) <= tft.width() && (mcu_y + mcu_h) <= tft.height())
    {
		...
		//Serial.print("mcu nr: ");	Serial.println(cnt++);
		// Push all MCU pixels to the TFT window
		tft.pushColors(pImg, mcu_pixels);
	}
	else if ((mcu_y + mcu_h) >= tft.height()) {
		Serial.println("JpegDec.abort()");
		JpegDec.abort(); // Image has run off bottom of screen so abort decoding
	} else {
		Serial.print("MCU out of display area: ");
		sprintf(s, "mcu_w: %u, mcu_h: %u, mcu_x: %u, mcu_y: %u ", mcu_w, mcu_h, mcu_x, mcu_y); Serial.println(s);
	}
	cnt++;
  }
...
void renderScaledJPEG(int xpos, int ypos, uint8_t scale) {
  ...
  while (JpegDec.read()) {    // While there is more data in the file
    ...
    int16 mcu_x = JpegDec.MCUx * mcu_w / scale + xpos; // Calculate coordinates of top left corner of current MCU
    int16 mcu_y = JpegDec.MCUy * mcu_h / scale + ypos;
